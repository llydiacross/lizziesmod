...
namespace LizziesMod
{
    public static class ModProfileManager
    {
        ...
        public static bool VerifyLevelProfile(string path, out string warningText)
        {
            ...
            {
                Logger.Error($"[ModProfileManager] Error verifying profile: {e.Message}");
            }
    ...
    public class GameManager_StartGame_Patch
    {
        ...
        public static bool Prefix(GameManager __instance, out bool __state)
        {
            ...
            if (File.Exists(profilePath))
            {
                if (ModProfileManager.VerifyLevelProfile(profilePath, out string mismatchWarning))
                {
                    XUiC_MessageBoxWindowGroup.ShowOkCancel(
                        ...
                        () =>
                        {
                            Logger.Info("[ModProfileManager] Load aborted due to mismatch profile. Quitting to desktop.");
            ...
            XUiC_MessageBoxWindowGroup.ShowOkCancel(
                ...
                () =>
                {
                    Logger.Info("[ModProfileManager] Aborted load sequence. Quitting to desktop.");
                    UnityEngine.Application.Quit();
                }
            );
    ...
    public class GameManager_RequestToSpawnPlayer_Patch
    {
        public static void Postfix(ClientInfo _cInfo)
        {
            if (_cInfo != null && SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
            {
                ...
                Logger.Info($"[ModProfileManager] Sent active mod profile and synced settings to connecting player: {_cInfo.playerName}");
            }
        }
    }

    // Specific Weather System Patch... Weather mod patch to prevent crashes caused by orphaned weather data in save files when a mod is disabled or removed.

    [HarmonyPatch(typeof(BiomeDefinition), "SetWeatherGroup")]
    public class SaveRescue_WeatherGroup_Patch
    {
        ...
        public static Exception Finalizer(Exception __exception)
        {
            if (__exception != null)
            {
                ...
                Logger.Warning("[Weather Mod Patch] Suppressed weather crash! A disabled mod left orphaned weather data in the save file.");
    ...
    public class GameManager_SaveWorld_Patch
    {
        public static void Postfix()
        {
            ...
            {
                Logger.Warning("[ModProfileManager] Failed to save Level Mod Profile during world save. This may indicate a world that has just been created.");
            }
